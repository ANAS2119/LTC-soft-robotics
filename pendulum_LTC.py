...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
wiring = RandomWiring(input_dim=2, output_dim=2, neuron_count=64)
model  = LTCRNN(wiring, input_dim=2, hidden_dim=64, output_dim=2).to(device)
...
# ================================
#--Model Loading
model.load_state_dict(torch.load("best_LTC_pendulum.pth", map_location=device))
model.eval()
...
idx = 200   # you can change to any sequence index from 0 to 8099
