function results = validateLSTMModel(inputFile, outputFile, timeFile, makePlots, doPerturbation)
% Validate LSTM model using dataset
% Normal mode:
%   - read u, x_true, time
...
%   LSTMStateFcnFast(h,u)
%   lstmOutputFcnFast(h,u)
...
h = zeros(768,1);
for k=1:5
    h=LSTMStateFcnFast(h,uu(:,k));
end
% Simulation
x_pred = zeros(6,M+1);
x_pred(:,1) =lstmOutputFcnFast( h, zeros(3,1));

for k = 1:M

    u = uu(:,k);

    h=LSTMStateFcnFast(h,u); % 8x1
    x_pred(:,k+1) = lstmOutputFcnFast(h,u);
...
if doPerturbation
    ...
    x_pred_pert(:,1) = lstmOutputFcnFast(h, zeros(3,1));

    for k = 1:M
        u = uu_pert(:,k);
        h = LSTMStateFcnFast(h, u);
        x_pred_pert(:,k+1) = lstmOutputFcnFast(h, u);
    end
...
if makePlots
    ...
    sgtitle('LSTM,  Validation: Measured vs Predicted')

    figure
    bar(maeVec)
    title('MAE of LSTM Model')
    grid on
    ...
    if doPerturbation
        ...
        sgtitle('LSTM Predictions: Clean vs Perturbed Input')
        ...
        title('LSTM, Drift Magnitude Over Time')
        ...
        xlabel('time (s)')
        ylabel('||x_{pert}||_2')
        title('LSTM, Perturbed Prediction Norm (Stability Check)')
        ...
        xlabel('time (s)')
        ylabel('Sensitivity')
        title('LSTM, Sensitivity to Input Perturbation')

        figure;
        bar(meanAbsDiff)
        grid on
        title('LSTM, Mean |First Difference| per Output')
        ylabel('mean |dx|')
    end
end
