function results = validateLTCModel(inputFile, outputFile, timeFile, makePlots, doPerturbation)
% Validate LTC model using dataset
%Steps:
    % read u, x-true and time
...
%% Initialize LTC state (8 states)

M = length(timeT)-1;

% First 6 states = physical outputs, last 2 = hidden memory
h = zeros(8,1);
for k=1:5
    h=LTCStateFcnFast(h,uu(:,k));
end
h(1:6) = xx(:,1); % True X values
...
for k = 1:M
    ...
    h=LTCStateFcnFast(h,u); % 8x1
    x_pred(:,k+1)=lnnOutputFcnFast(h,u);
...
if doPerturbation
    ...
    x_pred_pert(:,1) = lnnOutputFcnFast(h, zeros(3,1));

    for k = 1:M
        u = uu_pert(:,k);
        h = LTCStateFcnFast(h, u);
        x_pred_pert(:,k+1) = lnnOutputFcnFast(h, u);
    end
...
if makePlots
    ...
    sgtitle('LTC,  Validation: Measured vs Predicted')

    figure
    bar(maeVec)
    title('MAE of LTC Model')
    grid on
    ...
    if doPerturbation
        ...
        sgtitle('LTC Predictions: Clean vs Perturbed Input')
        ...
        title('LTC, Drift Magnitude Over Time')
        ...
        xlabel('time (s)')
        ylabel('||x_{pert}||_2')
        title('LTC, Perturbed Prediction Norm (Stability Check)')
        ...
        xlabel('time (s)')
        ylabel('Sensitivity')
        title('LTC, Sensitivity to Input Perturbation')

        figure;
        bar(meanAbsDiff)
        grid on
        title('LTC, Mean |First Difference| per Output')
        ylabel('mean |dx|')
    end
end
