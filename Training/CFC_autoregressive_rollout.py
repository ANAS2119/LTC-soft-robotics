...
# Rebuild the CFC model
cell = CFCImprovedCell(input_dim=9, hidden_dim=64)
CFC_model = CFCRNN(cell, input_dim=9, hidden_dim=64, output_dim=6).to(device)

# Load CFC weights
state_dict = torch.load("soft_robotics_CFC_model.pth", map_location=device)
CFC_model.load_state_dict(state_dict)
CFC_model.eval()
...
def autoregressive_rollout_from_init_pose(
    model, data, dt=0.05, device="cuda", scaler_Y=None
):
    """
    ...
    with torch.no_grad():
        for t in range(rollout_horizon):
            ...
            y_hat = model(window, timespans)      # [1, seq_len, 6]
...
preds1, true1 = autoregressive_rollout_from_init_pose(
    model=CFC_model, data=Data1, dt=0.05, device="cuda", scaler_Y=scaler_Y)


preds2, true2 = autoregressive_rollout_from_init_pose(
    model=CFC_model, data=Data2, dt=0.05, device="cuda", scaler_Y=scaler_Y)

preds3, true3 = autoregressive_rollout_from_init_pose(
    model=CFC_model, data=Data3, dt=0.05, device="cuda", scaler_Y=scaler_Y)

preds4, true4 = autoregressive_rollout_from_init_pose(
    model=CFC_model, data=Data4, dt=0.05, device="cuda", scaler_Y=scaler_Y)
##############################################################################
